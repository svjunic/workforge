...
既存タスクを `VISUAL` または `EDITOR` に設定されたエディタで開き、タイトルと説明を更新します。最初の Markdown H1 をタスクタイトル、本文全体をタスク説明として保存します。実行中または削除済みのタスクは編集できません。変更は次回の `wf run` または `wf resume` で反映され、既存の tmux プロセスには反映されません。
